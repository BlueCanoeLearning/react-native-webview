...
- (void) forceIgnoreSilentSwitch:(BOOL)initialSetup {
  ...
  NSString *injectedJs;
  if (initialSetup) {
    injectedJs = @"var s=new Audio('\(silenceMono56kbps100msBase64Mp3)');s.id='wkwebviewAudio';s.controls=false;s.loop=true;s.play();document.body.appendChild(s)";
  } else {
    injectedJs = @"var s=document.getElementById('wkwebviewAudio');s.src=null;s.parentNode.removeChild(s);s=null;s=new Audio('\(silenceMono56kbps100msBase64Mp3)');s.id='wkwebviewAudio';s.controls=false;s.loop=true;s.play();document.body.appendChild(s)";
  }
